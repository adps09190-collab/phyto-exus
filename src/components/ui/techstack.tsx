...
> & {
  ...
  label?: string;
  /** Called when a tile is tapped/clicked (pointer up on touch, down on mouse). */
  onTileSelect?: (index: number) => void;
  /** Keep the swell/lift animation even when the OS asks for reduced motion. */
  forceMotion?: boolean;
};
...
export function ToolDock({
  ...
  const still = forceMotion ? false : (useReducedMotion() ?? false);
      ...
      >
        ...
        <motion.ul
          ...
          onPointerUp={(event) => {
            ...
            if (onTileSelect && current.current !== null) onTileSelect(current.current);
          }}
          onClick={() => {
            if (onTileSelect && current.current !== null) onTileSelect(current.current);
          }}
